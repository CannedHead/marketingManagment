...
module.exports = {
    ...
    createUser: function(req, res, next) {    
        ...
        if (req.body.lname === undefined) {
            console.log('missing parameter:last name');
            return next("lname not found")
        }
        if (req.param('email') === undefined  ) {
            console.log('missing parameter:email');
            return next("email not found")
        }
        if (req.param('departamento') === undefined  ) {
            console.log('missing parameter:email');
            return next("email not found")
        }
        if (req.param('ciudad') === undefined  ) {
            console.log('missing parameter:email');
            return next("email not found")
        }
    ...
    readUserByNIT: function(nit, callback) {
        
        User.findOne({ roles: { $nin: [ "admin" ] }, nit: nit}, function(err, rows) {
            if (err) throw err;
            // Execute callback
            callback(null, rows);
        });
